import { W as W3mFrameProviderSingleton } from "./reown__appkit.mjs";
import { m as createConnector, P as ProviderNotFoundError, C as ChainNotConfiguredError, c as getConnection, n as createConfig, o as watchPendingTransactions, p as watchConnections, e as watchConnection, q as injected, t as signMessage, u as prepareTransactionRequest, v as sendTransaction, x as waitForTransactionReceipt, y as writeContract, z as estimateGas, A as watchConnectors, D as reconnect, E as getConnections, F as connect, G as switchChain, H as getBalance, I as disconnect } from "./wagmi__core.mjs";
import { C as ConstantsUtil, e as NetworkUtil, P as PresetsUtil, U as UserRejectedRequestError$1, E as ErrorUtil$1 } from "./reown__appkit-common.mjs";
import { g as getAddress, S as SwitchChainError, n as numberToHex, U as UserRejectedRequestError, F as parseUnits, z as formatUnits, G as checksumAddress } from "./viem.mjs";
import { h as getPreferredAccountType, C as ChainController, a as ConnectorController, v as getActiveCaipNetwork, g as AlertController, r as SIWXUtil, S as StorageUtil, O as OptionsController, o as WcHelpersUtil, d as CoreHelperUtil, A as AdapterBlueprint, w as AssetController, W as WalletConnectConnector } from "./reown__appkit-controllers.mjs";
import { E as ErrorUtil, C as CaipNetworksUtil, H as HelpersUtil } from "./reown__appkit-utils.mjs";
import "./@walletconnect/universal-provider+[...].mjs";
import { a as subscribeKey, p as proxy } from "./valtio.mjs";
...
async function getSafeConnector(connectors) {
  if (CoreHelperUtil.isSafeApp()) {
    const { safe } = await import("./wagmi__connectors.mjs");
...
async function getBaseAccountConnector(connectors) {
  try {
    const { baseAccount } = await import("./wagmi__connectors.mjs");
...
async function getCoinbaseConnector(connectors) {
  try {
    const { coinbaseWallet } = await import("./wagmi__connectors.mjs");
...
class WagmiAdapter extends AdapterBlueprint {
  ...
  async getAccounts(params) {
    ...
    return Promise.resolve({
      accounts: chainId ? [...new Set(addresses || [address])]?.map((val) => CoreHelperUtil.createAccount({
  ...
  async addWagmiConnector(connector) {
    ...
    const isInjectedType = connector.type === ConstantsUtil.CONNECTOR_ID.INJECTED;
    const isBasicInjected = connector.id === ConstantsUtil.CONNECTOR_ID.INJECTED;
