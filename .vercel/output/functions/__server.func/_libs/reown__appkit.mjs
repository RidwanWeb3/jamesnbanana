...
import { U as UniversalProvider } from "./@walletconnect/universal-provider+[...].mjs";
import { s as setColorTheme, a as setThemeVariables } from "./reown__appkit-ui.mjs";
import { b as bs58 } from "./bs58.mjs";
import { O as OptionsController, A as AdapterBlueprint, W as WalletConnectConnector, d as CoreHelperUtil, e as ConstantsUtil, C as ChainController, f as ApiController, g as AlertController, B as BlockchainApiController, P as ProviderController, h as getPreferredAccountType, a as ConnectorController, S as StorageUtil, i as ConnectionController, E as EnsController, j as AssetUtil, k as ConnectionControllerUtil, l as ConnectorUtil, m as OnRampController, M as ModalController, n as SendController, o as WcHelpersUtil, p as EventsController, q as AdapterController, T as ThemeController, r as SIWXUtil, R as RouterController, s as PublicStateController, t as SnackController, u as WalletUtil, v as getActiveCaipNetwork } from "./reown__appkit-controllers.mjs";
import { c as W3mFrameProvider, d as W3mFrameHelpers, W as W3mFrameRpcConstants } from "./reown__appkit-wallet.mjs";
import { C as ConstantsUtil$1, b as ParseUtil, e as NetworkUtil, c as getW3mThemeVariables } from "./reown__appkit-common.mjs";
import { E as toHex } from "./viem.mjs";
import { E as ErrorUtil, T as TokenUtil, C as CaipNetworksUtil, a as ConstantsUtil$2, H as HelpersUtil, L as LoggerUtil } from "./reown__appkit-utils.mjs";
import { t } from "./lit__react.mjs";
import { A as AppKitButton, a as AppKitNetworkButton, b as AppKitConnectButton, c as AppKitAccountButton } from "./reown__appkit-scaffold-ui.mjs";
...
class UniversalAdapter extends AdapterBlueprint {
  ...
  async getBalance(params) {
    const isBalanceSupported = params.caipNetwork && ConstantsUtil.BALANCE_SUPPORTED_CHAINS.includes(params.caipNetwork?.chainNamespace);
  ...
  async signMessage(params) {
    ...
    let signature = "";
    if (ChainController.state.activeCaipNetwork?.chainNamespace === ConstantsUtil$1.CHAIN.SOLANA) {
  ...
  async switchNetwork(params) {
    ...
    const connector = this.getWalletConnectConnector();
    if (caipNetwork.chainNamespace === ConstantsUtil$1.CHAIN.EVM) {
      try {
...
class AppKitBaseClient {
  constructor(options) {
    ...
    this.getConnections = (namespace) => {
      if (!this.remoteFeatures.multiWallet) {
        AlertController.open(ConstantsUtil$1.REMOTE_FEATURES_ALERTS.MULTI_WALLET_NOT_ENABLED.DEFAULT, "info");
        return [];
      }
  ...
  async initialize(options) {
    ...
    if (OptionsController.state.features?.reownAuthentication || OptionsController.state.remoteFeatures?.reownAuthentication) {
      const { ReownAuthentication } = await import("./reown__appkit-controllers.mjs").then(function(n) {
        return n.V;
      });
  ...
  async openSend(args) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const unsubscribeModal = ModalController.subscribe((modal2) => {
        if (!modal2.open) {
          cleanup();
  ...
  async checkAllowedOrigins() {
    try {
      ...
      const isOriginAllowed = WcHelpersUtil.isOriginAllowed(currentOrigin, allowedOrigins, ConstantsUtil$1.DEFAULT_ALLOWED_ANCESTORS);
      if (!isOriginAllowed) {
  ...
  initializeOptionsController(options) {
    ...
    const evmAdapter = options.adapters?.find((adapter) => adapter.namespace === ConstantsUtil$1.CHAIN.EVM);
    if (evmAdapter) {
  ...
  createClients() {
    this.connectionControllerClient = {
      ...
      disconnect: async (params) => {
        ...
        const isAuth = connectorIdParam === ConstantsUtil$1.CONNECTOR_ID.AUTH || namespaceConnectorId === ConstantsUtil$1.CONNECTOR_ID.AUTH;
        const isWalletConnect = connectorIdParam === ConstantsUtil$1.CONNECTOR_ID.WALLET_CONNECT || namespaceConnectorId === ConstantsUtil$1.CONNECTOR_ID.WALLET_CONNECT;
        try {
  ...
  async onConnectExternal(params) {
    ...
    if (params.type === ConstantsUtil$2.CONNECTOR_TYPE_AUTH) {
      ...
      const hasConnectedAuthNamespace = authNamespaces.some((namespace2) => ConnectorController.getConnectorId(namespace2) === ConstantsUtil$1.CONNECTOR_ID.AUTH);
  ...
  getApprovedCaipNetworksData() {
    ...
    if (providerType === ConstantsUtil$2.CONNECTOR_TYPE_WALLET_CONNECT) {
  ...
  async syncNamespaceConnection(namespace) {
    try {
      if (namespace === ConstantsUtil$1.CHAIN.EVM && CoreHelperUtil.isSafeApp()) {
        ConnectorController.setConnectorId(ConstantsUtil$1.CONNECTOR_ID.SAFE, namespace);
      }
      const connectorId = ConnectorController.getConnectorId(namespace);
      this.setStatus("connecting", namespace);
      switch (connectorId) {
        case ConstantsUtil$1.CONNECTOR_ID.WALLET_CONNECT:
          await this.reconnectWalletConnect();
          break;
        case ConstantsUtil$1.CONNECTOR_ID.AUTH:
          break;
        default:
  ...
  async syncWalletConnectAccount() {
    ...
    const syncTasks = this.chainNamespaces.map(async (chainNamespace) => {
      ...
      if (sessionAddress) {
        ...
        if (this.caipNetworks && ChainController.state.activeCaipNetwork && adapter.namespace !== ConstantsUtil$1.CHAIN.EVM) {
        ...
        ConnectorController.setConnectorId(ConstantsUtil$1.CONNECTOR_ID.WALLET_CONNECT, chainNamespace);
        StorageUtil.addConnectedNamespace(chainNamespace);
  ...
  async syncAccount(params) {
    ...
    const isUnsupportedNetwork = ChainController.state.activeCaipNetwork?.name === ConstantsUtil$1.UNSUPPORTED_NETWORK_NAME;
    ...
    if (chainIdToUse) {
      ...
      if (network?.chainNamespace === ChainController.state.activeChain) {
        if (OptionsController.state.enableNetworkSwitch && !OptionsController.state.allowUnsupportedChain && ChainController.state.activeCaipNetwork?.name === ConstantsUtil$1.UNSUPPORTED_NETWORK_NAME) {
  ...
  syncConnectedWalletInfo(chainNamespace) {
    ...
    if (providerType === ConstantsUtil$2.CONNECTOR_TYPE_ANNOUNCED || providerType === ConstantsUtil$2.CONNECTOR_TYPE_INJECTED) {
      ...
      if (connectorId === ConstantsUtil$1.CONNECTOR_ID.COINBASE_SDK || connectorId === ConstantsUtil$1.CONNECTOR_ID.COINBASE || connectorId === ConstantsUtil$1.CONNECTOR_ID.BASE_ACCOUNT) {
        const connector = this.getConnectors().find((c) => c.id === connectorId);
        const defaultName = connectorId === ConstantsUtil$1.CONNECTOR_ID.BASE_ACCOUNT ? "Base Account" : "Coinbase Wallet";
        ...
        WcHelpersUtil.listenWcProvider({
          ...
          onConnect: (accounts) => {
            ...
            for (const namespace2 of this.chainNamespaces) {
              StorageUtil.removeDisconnectedConnectorId(ConstantsUtil$1.CONNECTOR_ID.WALLET_CONNECT, namespace2);
            }
          ...
          onChainChanged: (chainId) => {
            ...
            const isCurrentConnectorWalletConnect = activeNamespace && ConnectorController.state.activeConnectorIds[activeNamespace] === ConstantsUtil$1.CONNECTOR_ID.WALLET_CONNECT;
  ...
  getAccount(_namespace) {
    ...
    return {
      ...
      embeddedWalletInfo: authConnector && activeConnectorId === ConstantsUtil$1.CONNECTOR_ID.AUTH ? {
  ...
  subscribeNetwork(callback) {
    return ChainController.subscribe(({ activeCaipNetwork, activeChain, chains }) => {
      const networkState = activeChain ? chains.get(activeChain)?.networkState : void 0;
      callback({
        caipNetwork: activeCaipNetwork,
...
class AppKit extends AppKitBaseClient {
  ...
  async onAuthProviderConnected(user) {
    ...
    const caipAddress = namespace === ConstantsUtil$1.CHAIN.EVM ? `eip155:${user.chainId}:${user.address}` : `${user.chainId}:${user.address}`;
  ...
  async syncAuthConnector(provider, chainNamespace) {
    const isAuthSupported = ConstantsUtil$1.AUTH_CONNECTOR_SUPPORTED_CHAINS.includes(chainNamespace);
    ...
    if (chainNamespace && isAuthSupported && shouldSync) {
      if (isConnected && this.connectionControllerClient?.connectExternal) {
        ...
        await this.connectionControllerClient?.connectExternal({
          ...
          info: { name: ConstantsUtil$1.CONNECTOR_ID.AUTH },
          type: ConstantsUtil$2.CONNECTOR_TYPE_AUTH,
          provider,
      ...
      } else if (ConnectorController.getConnectorId(chainNamespace) === ConstantsUtil$1.CONNECTOR_ID.AUTH) {
        this.setStatus("disconnected", chainNamespace);
        StorageUtil.removeConnectedNamespace(chainNamespace);
      }
    }
  ...
  createAuthProvider(chainNamespace) {
    const isSupported = ConstantsUtil$1.AUTH_CONNECTOR_SUPPORTED_CHAINS.includes(chainNamespace);
    if (!isSupported) {
      return;
    }
  ...
  async switchCaipNetwork(caipNetwork) {
    ...
    if (isSameNamespace && ChainController.getAccountData(networkNamespace)?.caipAddress) {
      ...
      const isCurrentNamespaceAuthProvider = currentNamespaceProviderType === ConstantsUtil$2.CONNECTOR_TYPE_AUTH;
      const newNamespaceProviderType = ProviderController.getProviderId(networkNamespace);
      const isNewNamespaceAuthProvider = newNamespaceProviderType === ConstantsUtil$2.CONNECTOR_TYPE_AUTH;
      const isNewNamespaceSupportsAuthConnector = ConstantsUtil$1.AUTH_CONNECTOR_SUPPORTED_CHAINS.includes(networkNamespace);
      if (!networkNamespace) {
      ...
      } else if (newNamespaceProviderType === ConstantsUtil$2.CONNECTOR_TYPE_WALLET_CONNECT) {
        if (!ChainController.state.noAdapters) {
          const adapter = this.getAdapter(networkNamespace);
          await adapter?.switchNetwork({ caipNetwork });
        }
  ...
  async syncIdentity({ address, chainId, chainNamespace }) {
    ...
    const isAuthConnector = ConnectorController.getConnectorId(chainNamespace) === ConstantsUtil$1.CONNECTOR_ID.AUTH;
    try {
  ...
  syncConnectedWalletInfo(chainNamespace) {
    const providerType = ProviderController.getProviderId(chainNamespace);
    if (providerType === ConstantsUtil$2.CONNECTOR_TYPE_AUTH) {
  ...
  async injectModalUi() {
    ...
    const features = { ...ConstantsUtil.DEFAULT_FEATURES, ...this.options.features };
  ...
  async loadModalComponents(features, remoteFeatures) {
    ...
    if (usingEmbeddedWallet) {
      featureImportPromises.push(import("./reown__appkit-scaffold-ui.mjs").then(function(n) {
        return n.e;
      }));
    }
    ...
    if (features.pay || remoteFeatures.payments) {
      featureImportPromises.push(import("./reown__appkit-pay.mjs"));
    }
    ...
    await Promise.all([
      ...
      import("./reown__appkit-scaffold-ui.mjs").then(function(n) {
        return n.i;
      }),
      import("./reown__appkit-scaffold-ui.mjs").then(function(n) {
        return n.w;
...
export {
  W3mFrameProviderSingleton as W,
  createAppKit as c
};
